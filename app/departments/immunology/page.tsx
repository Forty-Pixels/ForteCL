import { DEPARTMENTS_DATA } from '@/constants/departments';
import DepartmentDetailView from '@/components/Departments/Details/DepartmentDetailView';
import Footer from '@/components/Landing-page/Footer/Footer';
import { notFound } from 'next/navigation';

export const metadata = {
    title: 'Immunology Department | Forte Clinical Laboratory',
    description: 'Specialized testing for immune function, autoimmune disorders, and allergies.',
};

export default function ImmunologyPage() {
    const department = DEPARTMENTS_DATA.find(d => d.id === 'immunology');
    if (!department) notFound();

    return (
        <main className="min-h-screen bg-white">
            <DepartmentDetailView department={department} />
            <Footer />
        </main>
    );
}
