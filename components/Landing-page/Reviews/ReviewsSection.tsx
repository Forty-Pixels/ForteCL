                ...
                <div className="relative w-full overflow-hidden mask-fade-edges">
                    <div className="flex w-fit animate-reviews-marquee hover:pause-marquee gap-6 py-2">
                        {marqueeItems.map((review, index) => (
                            <div 
                                key={`${review.id}-${index}`}
                                className="flex-shrink-0 w-[300px] sm:w-[320px] h-[340px] bg-white p-5 rounded-[1.5rem] shadow-sm border border-gray-100 hover:shadow-lg hover:border-[#307984]/20 transition-all duration-300 relative group flex flex-col justify-between"
                            >
                                {/* Google Logo Shadow */}
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-[#202020] text-xs leading-tight">{review.name}</h4>
                                            <p className="text-[8px] text-gray-400 font-medium uppercase tracking-wider">{review.role} • {review.date}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex mb-2">
                                        {[...Array(review.rating)].map((_, i) => (
                                            <StarIcon key={i} className="w-3 h-3 text-[#f88c29]" />
                                        ))}
                                    </div>

                                    {/* Review Text */}
                                    <p className="text-[11px] text-gray-600 leading-relaxed italic line-clamp-12">
                                        "{review.text}"
                                    </p>
                                </div>
                                ...
                                <div className="mt-1 pt-2 border-t border-gray-50">
                                    <span className="text-[8px] text-gray-400 flex items-center gap-1 font-bold uppercase tracking-widest">
